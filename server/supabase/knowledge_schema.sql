...
alter table public.files
  add constraint files_user_fk foreign key (user_id) references auth.users (id) on delete cascade;
...
alter table public.knowledge_entries
  add constraint knowledge_entries_user_fk foreign key (user_id) references auth.users (id) on delete cascade;
...
create policy "Files can be managed by owner" on public.files
  for all
...
create policy "Knowledge entries readable by owner" on public.knowledge_entries
  for select
  using (auth.uid() = user_id);

create policy "Knowledge entries managed by owner" on public.knowledge_entries
  for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);
