...
router.post('/ai/generate', async (req, res) => {
  ...
  // Determine model: request override > cfg.model
  // Determine model: request override (body/query/header) > cfg.model > DEFAULT_MODEL env/config > fallback
  ...
  if (!OPENAI_API_KEY || !BASE_URL) {
    console.error('Missing OPENAI_API_KEY or BASE_URL in config');
...
router.post('/roleplay/character', async (req, res) => {
  ...
  const prompt = renderTemplate(cfg.characterGenerationTemplate, { theme });
...
router.post('/story/chapter', async (req, res) => {
  ...
  if (!previousChoice || chapterNumber === 1) {
    ...
    templateVars = { theme, previousChoice };
  }
